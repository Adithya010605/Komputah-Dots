...
TEXT_ICON="${TEXT_ICON:-$HOME/.config/rofi/icons/clipboard-text.svg}"
...
build_rofi_input() {
  local i line preview decoded mime ext img bytes type clean text_file
  ...
  for i in "${!raw_entries[@]}"; do
    ...
    clean="${clean//$'\r'/ }"
    clean="${clean//$'\t'/ }"
    text_file="$tmpdir/entry-$i.txt"
    if printf '%s' "$line" | cliphist decode >"$text_file" 2>/dev/null; then
      bytes=$(wc -c <"$text_file" | tr -d ' ')
    else
      bytes=${#preview}
    fi
    printf '%s\0icon\x1f%s\n' "$(truncate_preview "$clean")" "$TEXT_ICON"
  done
}
