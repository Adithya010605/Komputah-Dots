...
// The pomodoro backend stays the shell script — it owns the session log and
// the notifications, and the bar is just another face on it.
Singleton {
    id: root
    ...
    // What the bar module shows: the countdown while there is one, and the
    // idle hourglass otherwise.
    readonly property string barText: running ? formatTime(timeLeft) : "󰔟"
    ...
    // Periodic resync: picks up stats, preset changes and anything done to the
    // timer from outside the shell.
    Timer {
        interval: 5000
        running: true
        repeat: true
